...
def test_decode_unknown():
    ...
    assert frame.cmd == 8
    assert frame.port == 5
    assert frame.payload == b"unknown command payload"
...
def test_decode_data():
    ...
    assert frame.payload == b"this is a data frame"
...
def test_encode_unknown():
    ...
    """
    assert bytes(KISSCommand(port=3, cmd=12, payload=b"testing")) == (
        b"\x3ctesting"
    )
...
def test_encode_return():
    ...
    """
    assert bytes(KISSCmdReturn()) == b"\xff"
...
def test_encode_data():
    ...
    """
    assert bytes(KISSCmdData(port=2, payload=b"a frame")) == b"\x20a frame"
