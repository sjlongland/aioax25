...
class OneshotSlot(Slot):
    ...
    def __call__(self, *args, **kwargs):
        super(OneshotSlot, self).__call__(*args, **kwargs)
        signal = self._signal()
        if signal:
...
class Signal(BaseSignal):
    ...
    def connect(self, slot, *args, **kwargs):
        ...
        """
        super(Signal, self).connect(Slot(slot, *args, **kwargs))

    def connect_oneshot(self, slot, *args, **kwargs):
        """
        Connect a slot to the signal, and call it exactly once when the
        signal fires.  (Disconnect after calling.)
        """
        super(Signal, self).connect(OneshotSlot(self, slot, *args, **kwargs))
