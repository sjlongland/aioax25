...
class AX25Peer(object):
    ...
    def _on_receive_sabm(self, frame):
        ...
        self._log.debug(
            "Received SABM(E): %s (extended=%s)", frame, modulo128
        )
        if modulo128:
        ...
        if (frame.fi == frame.FI) and (frame.gi == frame.GI):
            for param in frame.parameters:
                if param.pi == AX25XIDParameterIdentifier.ClassesOfProcedure:
                    ...
                    param.pi == AX25XIDParameterIdentifier.IFieldLengthReceive
                ):
                    self._process_xid_ifieldlenrx(param)
                elif param.pi == AX25XIDParameterIdentifier.WindowSizeReceive:
                    self._process_xid_winszrx(param)
                elif param.pi == AX25XIDParameterIdentifier.AcknowledgeTimer:
                    self._process_xid_acktimer(param)
                elif param.pi == AX25XIDParameterIdentifier.Retries:
