...
class APRSSexagesimal(object):
    ...
    def __repr__(self):  # pragma: no cover
        return "%s(degrees=%r, minutes=%r, seconds=%r, ambiguity=%r)" % (
...
class APRSUncompressedCoordinates(object):
    ...
    def decode(cls, coordinate):
        ...
        symbol = APRSSymbol(coordinate[APRSLatitude.LENGTH], coordinate[-1])
...
class APRSCompressionType(object):
    ...
    def __repr__(self):  # pragma: no cover
        return "%s(gpsfix=%r, nmeasrc=%r, origin=%r)" % (
            self.__class__.__name__,
            self.gpsfix,
            self.nmeasrc,
            self.origin,
...
class APRSCompressedCoordinate(APRSSexagesimal):
    ...
    def decode(cls, coordinate):
        ...
        value = (
            (decompress(coordinate) / cls.POSTSCALE) - cls.OFFSET
        ) / cls.PRESCALE
...
class APRSCompressedCourseSpeedRange(object):
    ...
    def decode(cls, csvalue, ctype):
        ...
        if ctype.nmeasrc == APRSCompressionTypeNMEASrc.GGA:
            ...
            altitude = None
            csvalue = [
                b - BYTE_VALUE_OFFSET for b in bytes(csvalue, "us-ascii")
            ]
            ...
            if csvalue[0] == cls.RANGE_HEADER:
                ...
                raise ValueError(
                    "Unknown Course/Speed/Range field: %r" % csvalue
                )
    ...
    def __init__(self, course=None, speed=None, rng=None, altitude=None):
        ...
        if altitude is not None:
            ...
            if (course is None) or (speed is None):
                raise ValueError("Course and speed must both be specified")
    ...
    def __repr__(self):  # pragma: no cover
        return "%s(course=%r, speed=%r, rng=%r, altitude=%r)" % (
            self.__class__.__name__,
            self.course,
            self.speed,
            self.rng,
    ...
    def __str__(self):
        if self.altitude is not None:
            return compress(
                int(math.log(self.altitude, self.ALTITUDE_RADIX)), self.LENGTH
            )
        elif self.rng is not None:
            # Return range
            bvalues = [
                self.RANGE_HEADER,
                int(math.log(self.rng / self.RANGE_SCALE, self.RANGE_RADIX)),
            ]
        else:
            # Return speed/course
            bvalues = [
                int(self.course / self.COURSE_SCALE),
                int(math.log(self.speed + 1, self.SPEED_RADIX)),
            ]
...
class APRSCompressedCoordinates(object):
    ...
    def decode(cls, coordinate):
        ...
        if coordinate[10] == cls.CST_FILL[0]:
            ...
            csr = APRSCompressedCourseSpeedRange.decode(
                coordinate[-3:], ctype
            )
    ...
    def __repr__(self):  # pragma: no cover
        return "%s(lat=%r, lng=%r, symbol=%r, ctype=%r, csr=%r)" % (
            self.__class__.__name__,
            self.lat,
            self.lng,
            self.symbol,
...
class APRSPositionFrame(APRSFrame):
    ...
    def decode(cls, uiframe, payload, log):
        ...
        if msgtype in (
            ...
            payload = payload[8:]
        elif msgtype in (APRSDataType.POSITION, APRSDataType.POSITION_MSGCAP):
            position_ts = None
            payload = payload[1:]
        else:
    ...
    def __init__(
        ...
        payload = self._encodepayload(
            position, message, position_ts, messaging
        )
...
APRSFrame.DATA_TYPE_HANDLERS[APRSDataType.POSITION] = APRSPositionFrame
APRSFrame.DATA_TYPE_HANDLERS[APRSDataType.POSITION_MSGCAP] = APRSPositionFrame
APRSFrame.DATA_TYPE_HANDLERS[APRSDataType.POSITION_TS] = APRSPositionFrame
APRSFrame.DATA_TYPE_HANDLERS[
    APRSDataType.POSITION_TS_MSGCAP
] = APRSPositionFrame
